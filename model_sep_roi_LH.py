...
def crop_and_resize(image, boxes, box_ind, crop_size, pad_border=False):
    ...
    def transform_fpcoor_for_tf(boxes, image_shape, crop_shape):
        ...
        x0 *= tf.to_float(image_shape[1])
        y0 *= tf.to_float(image_shape[0])
        x1 *= tf.to_float(image_shape[1])
        y1 *= tf.to_float(image_shape[0])
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            #    method="bilinear"))
            ret =  crop_and_resize(feature_maps[i], level_boxes, box_indices, self.pool_shape[0]*2, pad_border=False)
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    #print(scale_factor)
    h,w = image.shape[:2]
    #pad_value = np.mean(np.mean(original_image,axis=0),axis=0)
    if augment:
    #    if scale_factor < 1:
    ...
    # Random horizontal flips.
        ...
        if random.randint(0, 1):
            edge = min(h,w)
            image,mask = transform.random_crop_transform2(image, mask, int(edge*scale_factor),int(edge*scale_factor), u=1)
        else:
            image,mask = transform.random_crop_transform2(image, mask, int(w*scale_factor),int(h*scale_factor), u=1)
        """
        image, mask = transform.random_shift_scale_rotate_transform2(image, mask,
                        shift_limit=[0,0], scale_limit=[0.8,1],
                        rotate_limit=[0,0], borderMode=cv2.BORDER_CONSTANT , u=0.5)
        specle_noise = iaa.Sometimes(
                       0.99,
                       iaa.MultiplyElementwise((0.975,1.025))
                       )
        image = specle_noise.augment_image(image)
    shape = image.shape
...
def large_separe_conv(inputs,k,name_prefix,cmid,cout):
    x1 =KL.Conv2D(cmid, (k, 1),padding="SAME", name=name_prefix + 'l1')(inputs)
    x1 = KL.Activation('relu')(x1)
    x1 = KL.Conv2D(cout, (1, k),padding="SAME", name=name_prefix + 'l2')(x1)
    x1 = KL.Activation('relu')(x1)

    x2 = KL.Conv2D(cmid, (1, k),padding="SAME", name=name_prefix + 'r1')(inputs)
    x2 = KL.Activation('relu')(x2)
    x2 = KL.Conv2D(cout, (k, 1),padding="SAME", name=name_prefix + 'r2')(x2)
    x2 = KL.Activation('relu')(x2)
    out = KL.add([x1,x2])
    return out
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        """
        P2 = large_separe_conv(P2,k=15,name_prefix="fpn_p2",cmid=64,cout=256)
        P3 = large_separe_conv(P3,k=15,name_prefix="fpn_p3",cmid=64,cout=256)
        P4 = large_separe_conv(P4,k=15,name_prefix="fpn_p4",cmid=64,cout=256)
        P5 = large_separe_conv(P5,k=15,name_prefix="fpn_p5",cmid=64,cout=256)
        """
        ...
        if mode == "training":
            ...
            model = KM.Model([input_image, input_image_meta],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 mrcnn_mask, rpn_rois, rpn_class, rpn_bbox,normalized_boxes,scores],
                             name='mask_rcnn')
    ...
    def find_last(self):
        ...
        return [dir_name,checkpoints], checkpoint
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window,return_mode="thres"):
        ...
        """
        assert return_mode in {"thres","raw"}
        ...
        for i in range(N):
            ...
            if return_mode == "thres":
                ...
                #print("return float")
                full_mask = utils.unmold_mask_raw(masks[i], boxes[i], image_shape)
    ...
    def detect(self, images,return_mode = "thres", verbose=0):
        ...
        detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, \
            rois, rpn_class, rpn_bbox,n_box,n_score =\
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks =\
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       image.shape, windows[i],return_mode)
            results.append({
                "rois": final_rois,
                "class_ids": final_class_ids,
                "scores": final_scores,
                "masks": final_masks,
                "non_supress_box":n_box,
                "non_supress_score":n_score
            })
        return results
